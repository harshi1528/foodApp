...
import '../Styles/TableBooking.css'; // Import CSS file for styling
...
const MyBookings = () => {
  ...
  useEffect(() => {
    const fetchBookings = async () => {
      try {
        ...
        const bookingsResponse = await axios.get(`https://localhost:7120/api/TableBookings`);
        ...
        const bookingsWithCustomers = await Promise.all(bookingsData.map(async (booking) => {
          ...
          const customerResponse = await axios.get(`https://localhost:7120/api/Customers/${booking.customerId}`);
  ...
  const handleSave = async () => {
    try {
      ...
      await axios.put(`https://localhost:7120/api/TableBookings/${editingBookingId}`, updatedBookingData);
  ...
  const handleDelete = async (bookingId) => {
    try {
      ...
      await axios.delete(`https://localhost:7120/api/TableBookings/${bookingId}`);
